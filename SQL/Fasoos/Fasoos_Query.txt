...
-- find driver id whose order get cancelled
select driver_id,count(distinct order_id) from driver_order 
where cancellation not in ('Cancellation', 'Customer Cancellation')
group by driver_id;

-- how many each type roll was delivered 
select roll_id ,count(roll_id) from
customer_orders where order_id in(
select order_id from
(select*,case when cancellation in ('Cancellation','Customer Cancellation')--where order got not cancelled
...
-- maximum no of roll delivered in a single order?


select*,rank() over(order by roll_count desc) rank from(
select order_id , count(roll_id) as roll_count from(
